...
- `상세내용` 탭: API 연결 + 디자인 세분화 완료, **지원서 첨부(required_files) 연동 완료**
- `리뷰` 탭: 조회·통계·필터·만족도 전체평균·**직무연관성 차트 실데이터** 연동 완료
- 리뷰 **작성**(3스텝+인증) + **수정(edit 모드)** PC/모바일 완료
- **활동 참여결과(캘린더)** results 실데이터 + 합격/수료 저장 + 리뷰작성 라우팅 완료
- 부가기능: 인기검색어·닉네임중복·알림/이메일동의 초기값·저장공고 진행여부 필터 완료
...
2. **PC 계정 알림설정 토글** — 현재 모바일(`MobileAccountPage`)에만 있음. PC(`PcAccountPage`)에도 인기공고/저장공고 토글 추가할지 디자인 확인(백엔드·훅은 준비됨, `me.notificationPreferences` + `PATCH /api/members/notifications {type}`).
3. **프로필 리뷰작성 유도 화면** — 3round-3 #55, figma **1136-75658**. 참여활동 있을 때 노출. 시안 PNG 필요.

### B. 실측/검증 권장 (참여·리뷰 데이터 있는 계정 필요)
