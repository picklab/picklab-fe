...
const FilterSection = ({
  ...
  const handleSelectFilter = (category: string, option: string) => {
    setSelectedFilters((prev) => {
      ...
      if (current.includes(option)) {
        newFilters[category] = current.filter((item) => item !== option);
      } else {
  ...
  const tabOptions = ACTIVITY_FILTERS.map((filter) => ({
    label: filter.title,
    value: filter.title,
  }));

  const selectedFilterEntries = Object.entries(selectedFilters).flatMap(
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-wrap gap-2.5">
        {currentOptions.map((option) => {
          const isSelected =
            selectedFilters[currentTab]?.includes(option) ?? false;
          return (
            <Button
              key={option}
              label={option}
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        {selectedFilterEntries.map(({ category, option }) => (
          <div
            key={`${category}-${option}`}
            className="flex items-center bg-white px-3 py-1.5 border border-solid gap-1 border-primary-60 rounded-full">
            <Typography type="Body3Medium" className="text-primary-60">
              {option}
            </Typography>
            <button onClick={() => handleSelectFilter(category, option)}>
              <Icon icon="xMark" color="#009966" size={18} />
            </button>
          </div>
        ))}
        <button
