...
export default function SignupPage() {
  ...
  const isCurrentStepValid = () => {
    switch (currentStep) {
      case 1:
        return (
          signupData.terms.age &&
          signupData.terms.service &&
          signupData.terms.privacy
        );
      case 2:
        return (
  ...
  const handleSignupComplete = async () => {
    const unsupportedInterests = signupData.interests.filter(
      (interest) => !JOB_CATEGORY_MAP[interest],
    );
    if (unsupportedInterests.length > 0) {
      window.alert(
        `현재 API에서 지원하지 않는 직무가 포함되어 있습니다: ${unsupportedInterests.join(", ")}`,
      );
      return;
    }
    ...
    try {
      ...
      const response = await fetch("/api/members/signup/additional-info", {
        ...
        body: JSON.stringify({
          ...
          education_level:
            EDUCATION_LABELS[signupData.userInfo.education] ??
            signupData.userInfo.education,
          school: signupData.userInfo.schoolName.trim(),
          graduation_status:
            GRADUATION_LABELS[signupData.userInfo.graduationStatus] ??
            signupData.userInfo.graduationStatus,
          employment_status: signupData.userInfo.employmentStatus
            ? (EMPLOYMENT_LABELS[signupData.userInfo.employmentStatus] ??
              signupData.userInfo.employmentStatus)
            : "",
          company: signupData.userInfo.companyName.trim(),
          employment_type: "NONE",
          interested_job_categories: signupData.interests.map(
            (interest) => JOB_CATEGORY_MAP[interest],
          ),
        }),
      });
      ...
      if (!response.ok) {
        window.alert(
          readErrorMessage(payload, "회원 추가 정보 저장에 실패했습니다."),
        );
        return;
      }
  ...
  return (
    <div className="flex flex-col gap-12 items-center max-w-[429px] mx-auto pt-12">
      ...
      <div className="relative flex justify-center gap-[14px]">
        <div
          className="absolute left-6 right-6 top-[14px] h-px bg-gray-20"
          aria-hidden="true"
        />
        {SIGNUP_ICONS.map((icon, index) => {
          ...
          return (
            <div
              key={index}
              className="relative z-10 flex w-12 flex-col items-center justify-center gap-[5.5px] bg-white">
              <Image
                src={isCurrent ? icon.activeSrc : icon.src}
                alt={icon.alt}
                width={28}
                height={28}
              />
      ...
      {currentStep !== 4 && (
        ...
        <button
          ...
          <Typography
            type="Heading2Medium"
            className={canProceed ? "text-gray-0" : "text-gray-50"}>
            {nextButtonLabel}
          </Typography>
        </button>
        </div>
